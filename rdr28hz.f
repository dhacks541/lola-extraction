c RDR28Hz is taken from RDR2CSV, but outputs the full rate switchout time, energy, and pulse width at 28Hz.
c Switchout time is ns from minor frame start, after 9.5 ms delay, energy in mJ, tx_PW,
c original RDR2CSV so lots of cruft here. 
c RDR2CSV output many more columns, including
c spacecraft position, height, geoid etc. 
c **********************************************************************/
c ************************************************************************
c uses "unsigned" and "kzext2" sign-extension subroutines written in C
c gfortran -ffixed-line-length-none -O rdr28hz.f unsigned.o kzext2.o -static-libgfortran -o rdr28hz
c v1.0
c
	IMPLICIT NONE
...
c minor frame counts as fractions of 5000000.d0
	...
	logical swap /.false./,head/.false./, all/.true./,geop/.false./
...
c initialize minor frame offsets - use to check range window against time
	...
	if(numarg.eq.0) then
	 write(*,*)'rdr28hz 1.0 - LOLA reduced data record 28-Hz laser table'
	 write(*,*)'MET_seconds,switchout time from pump to fire, Transmit energy, PW'
c	 write(*,*)'5-detector noise counts, detector thresholds, channel gain setting'
	 write(*,*)'usage: rdr28hz <rdrfile>'
	 goto 99999
	endif
...
c	 if(arg(1:1).eq.'u' .or. arg(1:1).eq.'U') flagu=.not.flagu!utc output
...
c	if (flagu) call ldpool('naif0012.tls')
c ======= limits
	if(limit) then
...
c	if(head)
	write(*,'(a,a,a)')
     &  'MET, Switchout,  Txenrg,  TxPw'
c     & ,',  solinc,  nois1,  nois2,  nois3,  nois4,  nois5, thrs1, thrs2, thrs3, thrs4, thrs5'
c     & ,', gain1, gain2, gain3, gain4, gain5,    sclon,    sclat'
c first record
	MET=rdr(1)
...
c	  write(*,1201) metold,tdtold,tickold,sumo/isum, isum, sumtx/isum, sumpw/isum, rngmx, solinc,noissm, thrs, gain,sclon,sclat
c tricky leading zero suppress to concatenate tdt and tick
1201	format(i10,",",i10,f0.6,",",9p,f10.1,",",i3,",",0p,2(f8.3,","),f8.2,",",f8.2,",",5(i7,","),10(f6.2,","),f9.4,",",f9.4)
c setup
...
c ***** laser fired, output csv record
	if( rdr(iwrd+6) .gt. 0.) then
	 write(*,'(f15.3,", ",6p,f10.3,", ",0p,f8.3,", ",f8.3)') met+fract, fract-offset,xenrg,xplse
c *****
c for averages
	 isum = isum +1
...
c   bit 15= weird
	...
	if(.not.limit.or.flagv
     ...
     &  ) then
c	if(flagt) write(*,*) rdr(iwrd+i*10+3),rdr(iwrd+i*10+4)
...
c	write(0,*)'irec,iwrd,rdr',irec,iwrd,rdr(iwrd+1-64),rdr(iwrd+2-64),rdr(iwrd+3-64),rdr(iwrd+4-64)
