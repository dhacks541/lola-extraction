c edr2hkEng 3424-byte format -> housekeeping at 1-s intervals
c V2.0
c gfortran -ffixed-line-length-none -O edr2hkEng.f zext.o kzext2.o kzext3.o ksext3.o jzext.o -o edr2hkEng
	implicit none
...
1	inquire(unit=1, POS=mypos)!points to next byte.
	close(1)
        irec=0
	if(head) then
	write(*,'(16A)')'MET_TIME,','J2000, Lsr, Enab, Fire, Rgstrt, Rgstop'
     & , ', Clk, RX1, RX2, RX3, RX4, RX5, V550, V5, V12, V3.3D, V3.3A, V5NEG' !out-of-order
     & , ', DIODE1T, DIODE2T'
     & , ', DU3D3IMONA, DU3D3IMOND, DU1D5IMONA, DU12IMON, DU1D5VMONA, DU1D5IMOND' !out-of-order
     & , ', HY_T1, HY_T2, HY_T3, HY_T4, HY_T5, DB_T1, DB_T2, DB_T3, DB_T4, DB_T5' !out-of-order
     & , ', LEA_T, L1DI_T, L2DI_T, L1B_T, L2B_T, PCA_T, ANBD_T, OSC_T, DUBD_T'
     & , ', BX_BOT, BX_MID, BX_TOP, RX_BOT, RX_MID, RX_TOP'
     & , ', C_HI_T, HSNG_T, DUA_T, DUA_HOT1_T, DUA_HOT2_T, DUA_FPGA_T'
     & , ', OVRStatus, Alg_mode'
...
c the LEA high order bits don't change
	write(*,'(i2,",",a7,",",a7,",",i6,",",i6,",",$)')lsr,lsrcnfst,lsrfires,rgstart,rgstop
...
c	write(*,'(F6.2,",",F7.2,",",$)') DIODE_CURRENT_SET,TX_THRESHOLD_READ_BACK
...
c hybrids 78,80,82,84,86
	...
	write(*,'(5(f8.2,","),$)')
     & HYB_TEMP_1,HYB_TEMP_2,HYB_TEMP_3,HYB_TEMP_4,HYB_TEMP_5
c boards 75,77,79,81,83
	...
	write(*,'(5(f8.2,","),$)')
     & DTB_TEMP_1,DTB_TEMP_2,DTB_TEMP_3,DTB_TEMP_4,DTB_TEMP_5
	...
	write(*,'(9(f8.3,","),$)')LEA_BOARD_TEMP,LASER_1_BENCH,LASER_2_BENCH,LASER_1_DIODE,LASER_2_DIODE,PCA_T,ANBD_T,OSC_T,DUBD_T
c lola tubes - could have switched in cal high/low in here?
	...
	write(*,'(6(f7.2,","),$)')
     & CALIBRATION_HIGH_TEMP,HOUSING_TEMP,DUA_TEMP,DUA_HOT1_TEMP,DUA_HOT2_TEMP,DUA_FPGA_TEMP
