...
c gfortran -ffixed-line-length-none -O rdr2utcLsr.f unsigned.o kzext2.o spicelib.a -static-libgfortran -o rdr2utcLsr
c v3.0 usage message
...
c initialize minor frame offsets - use to check range window against time
	...
	if(numarg.eq.0) then
	 write(*,*)'rdr2utc 2.1 - LOLA reduced data record to 1-Hz table'
...
c SPICE needs these if you uncomment et2utc
	if (flagu) call ldpool('naif0012.tls')
c ======= limits
	if(limit) then
...
c average for single MET is posted to the beginning of the frame after reading 28 records
	if (met.ne.metold) then !new frame
	  ...
	  write(*,1201) metold,tdtold,tickold,sumo/isum, isum, sumtx/isum, sumpw/isum, rngmx, solinc,offndr, sclon,sclat
c tricky leading zero suppress to concatenate tdt and tick
1201    format(i10,",",i10,f0.6,",",9p,f10.1,",",i3,",",0p,2(f8.3,","),f8.2,",",f8.2,",",f8.2,",",f9.4,",",f9.4,$)
c	format(i10,",",i10,f0.6,",",9p,f10.1,",",i3,",",0p,2(f8.3,","),f8.2,",",f8.2,",",f9.4,",",f9.
	if (flagu) then
	 tt=tdtold+tickold
	 call et2utc (tt,'isod',5,utc)
	 write(*,1202) utc
...
c scale factor accounts for km^2, mJ to f
...
c cleanup last record
	write(*,1201) met,tdtold,tickold,sumo/isum, isum, sumtx/isum, sumpw/isum, rngmx, solinc, sclon, sclat
	if (flagu) then
	 tt=tdtold+tickold
	 call et2utc (tt,'isod',5,utc)
	 write(*,1202) utc
